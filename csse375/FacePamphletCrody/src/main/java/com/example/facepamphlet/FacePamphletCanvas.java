...
public class FacePamphletCanvas extends GCanvas {
	...
	/* ADDED
	 * adding this for friend offset
	 */
	double friendOffset = 10;
	...
	public void displayProfile(FacePamphletProfile profile) {
		...
		this.addEmployment(profile.getEmployment(), profile.getEmploymentDates());
		this.addEducation(profile.getEducation(), profile.getEducationDates());
	}
	
	public void displayAdminPane(FacePamphletDatabase db){
		this.removeAll();
		addName("In Admin Mode, Names of all profiles are shown with their friends indented under them");
		this.showAllProfiles(db);
	}
	
	/* ADDED
	 * logic for displaying all the admin information
	 */
	private void showAllProfiles(FacePamphletDatabase db) {
		Set<String> keys = db.getKeySet();
		double y = TOP_MARGIN + this.nameHeight;
		for(String profileName : keys){
			GLabel name = new GLabel(profileName);
			name.setFont(PROFILE_NAME_FONT);
			name.setColor(Color.GRAY);
			add(name, LEFT_MARGIN, y += this.nameHeight);
			FacePamphletProfile profile = db.getProfile(profileName);
			Iterator<String> friendsIter = profile.getFriends();
			while(friendsIter.hasNext()){
				String friend = friendsIter.next();
				GLabel friendLabel = new GLabel(friend);
				add(friendLabel, LEFT_MARGIN + this.friendOffset, y += this.nameHeight);
			}
		}
	}
	...
	/* ADDED
	 * here is the logic behind displaying the education and employment
	 */
	private void addEmployment(Iterator<String> location, Iterator<String> date){
		GLabel employment = new GLabel("Employment:");
		employment.setFont(PROFILE_FRIEND_LABEL_FONT);
		double x = getWidth()/2;
		double y = getHeight()/4;
		add(employment, x, y);
		for(int i = 1; location.hasNext(); i++) {
			GLabel locationName = new GLabel(date.next() + " " + location.next());
			locationName.setFont(PROFILE_FRIEND_FONT);
			double height = y + employment.getHeight() * i;
			add(locationName, x, height);
		}
	}
	
	private void addEducation(Iterator<String> location, Iterator<String> date){
		GLabel education = new GLabel("Education:");
		education.setFont(PROFILE_FRIEND_LABEL_FONT);
		double x = getWidth()/2;
