...
public class FacePamphletProfile implements Serializable {
	...
	private FacePamphletStatus Status = new FacePamphletStatus("No current status", null);
	...
	public void setStatus(String status, String attachment) {
		if(this.statusSet){
			this.pastStatuses.add(this.Status);
		} else {
			this.statusSet = true;
		}
		this.Status = new FacePamphletStatus(status, attachment);
	}
