...
class CornersProblem(search.SearchProblem):
  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      hitsWall = self.walls[nextx][nexty]
      if not hitsWall:
        newCorners = list(set(cornersLeft) - set([(x,y)]))
        successors += [(((nextx, nexty), newCorners), action, 1)]
...
def getClosestPoint(start, ends):
  (x1, y1) = start
...
def cornersHeuristic(state, problem):
  ...
  "*** YOUR CODE HERE ***"
  (currentPoint, cornersLeft) = state
  ...
  while cornersLeft != []:
    (closest, dist) = getClosestPoint(currentPoint, cornersLeft)
    currentPoint = closest
    cornersLeft = list(set(cornersLeft) - set([closest]))
    val += dist
