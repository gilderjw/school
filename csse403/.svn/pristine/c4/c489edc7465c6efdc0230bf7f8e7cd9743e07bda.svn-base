...
print_with_parens(List) ->
    lists:foreach(fun(X) -> io:fwrite("(~B)", [X]) end, List).
...
reverse_all(ListOfLists) ->
    lists:map(fun(X) -> lists:reverse(X) end, ListOfLists).
...
member(Item,List) ->
    lists:foldr(fun(Ans, X) -> X or (Ans == Item) end, false,List).
...
subtract(List1,List2) ->
    lists:filter(fun(X) -> not list_probs:member(X, List2) end, List1).
...
append_strings(OrigString,ListOfStrings) ->
    lists:foldl(fun(Ans, Y) -> Y++Ans end, OrigString, ListOfStrings).
...
append_strings2(OrigString,ListOfStrings) ->
    lists:foldr(fun(Ans, Y) -> Ans++Y end, OrigString, ListOfStrings).
