...
getRandom : RandomState Float
getRandom = 
     (\seed -> (Random.step random seed))

--(>>=): (Seed -> (value, Seed)) -> (value -> (Seed -> (value, Seed))) -> (Seed -> (value, Seed))
(>>=): RandomState value -> (value -> RandomState newval) -> RandomState newval
(>>=) state next =
     (\initSeed -> 
          let (val, sed) = (state initSeed) in
               ((next val) sed))
...
main = let (output,seed) = doCode (initialSeed 2) in 
