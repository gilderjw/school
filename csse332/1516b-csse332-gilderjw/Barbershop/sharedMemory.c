/***************************************************************************
 * This module contains utility functions for managing share memory regions.
...
detachSharedMemory(sharedMemoryStruct *ShmPTR, char Who[]) {
  shmdt((void *)ShmPTR);
  fprintf(stderr, "%s has detached its shared memory...\n", Who);
}
...
removeSharedMemory(int ShmID, char Who[]) {
  ...
  fprintf(stderr, "%s has removed the shared memory region.\n", Who);
}
